...
class HashTable {
    ...
    ******************************************/
    ...
    void clear(Person[]);
    bool insHelper(Person[], Person);
    bool rmvHelper(Person[], Person, int);
    bool checkForInsRehash(const Person[]);
    bool checkForRmvRehash(const Person[]);
    int search(const Person[], const Person&);
    const HashTable& operator=(const HashTable& rhs); // overloaded assignment operator
