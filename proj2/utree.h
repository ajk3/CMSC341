...
private:
    ...
    bool recursiveInsert(UNode*, Account);
    void deleteNodes(UNode*);
    UNode* search(UNode*, string);
    void printInOrder(UNode*) const;

    UNode* findParent(UNode*, UNode*);  // finds parent
    void heightUpTheTree(UNode*);

    int findMaxHeight(UNode*);

    void imbalanceUpTheTree(UNode*);

    void removeAvlNode(UNode*);

    void printChild(UNode*);

    void removeUpdateHeight(UNode*);

    void leftRotation(UNode*&);

    void rightRotation(UNode*&);
};
