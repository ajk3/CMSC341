...
private:
    ...
    bool recursiveInsert(DNode*, Account); // helper function to recursively insert a node

    void recursiveClear(DNode*); // helper function to recursively insert a node
    
    DNode* findParent(DNode*, DNode*);  // finds parent
    
    DNode* search(DNode*, int); // searches for the key
    
    void deleteNodes(DNode*); // deletes nodes
    
    void sizeUpTheTree(DNode*);

    void vacantUpTheTree(DNode*);

    void imbalanceUpTheTree(DNode*&);

    void printInOrder(DNode*) const;

    void copyNodes(DNode*&, DNode* const);

    bool foundVacant(DNode*, Account); 

    int treeToArray(DNode*&, DNode*[], int);

    void arrayToTree(DNode*&, DNode*[], int, int);
};